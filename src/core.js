...
function getOperationFn(initialValue, operatorFn) {
    return operatorFn
        ? (newValue) => (initialValue = operatorFn(newValue, initialValue))
        : () => initialValue;
}
...
function deepEqual(firstObject, secondObject) {
    ...
    for (let property in firstObject) {
        if (
            !(property in secondObject) ||
            !deepEqual(firstObject[property], secondObject[property])
        ) {
            return false;
        }
    }

    return firstObjectKeysAmount == secondObjectKeysAmount;
}
